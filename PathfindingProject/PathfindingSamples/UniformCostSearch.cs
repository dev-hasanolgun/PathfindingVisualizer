...
/// Guarantees shortest path by total weighted cost.
/// </summary>
public class UCS
{
    ...
    /// <summary>
    /// Executes the UCS algorithm to find the shortest path.
    /// </summary>
    ...
    public Stack<Node> FindPath(Point start, Point goal, Point gridSize)
    {
        ...
        frontier.Enqueue(startNode, startNode.GCost);   // Add start node to exploration queue.
        ...
        while (frontier.Count > 0)
        {
            ...
            foreach (var neighbor in SampleUtils.GetNeighbors(currentNode.Point, gridSize, NeighborSearchType.FourWay))
            {
                ...
                if (neighborNode.State == NodeState.Unvisited || costToNeighbor < neighborNode.GCost)
                {
                    ...
                    frontier.Enqueue(neighborNode, neighborNode.GCost); // Add neighbor node to the frontier queue to be explored next.
                }
            }
        }
